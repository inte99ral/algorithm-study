...
cpu 아키텍쳐에 따라 i686 인지 x86인지, 64-bit 인지 32-bit 인지 구분합니다.

##### i686

&nbsp; i386(Intel 의 80386 CPU 계열의 CPU), i586(Intel 의 80586 CPU 계열의 Pentium CPU), i686(Intel 의 80686 CPU 계열의 Pentium Pro 이상의 CPU) 등 인텔 CPU 칩셋 중에서 i686에 호환되는 32비트의 시스템을 의미합니다.

##### x86_64

&nbsp; x86 은 i386, i586, i686 모든 ~86 플랫폼에서 동작하는 것을 의미합니다.

x86_64 는 x86의 64비트 컴퓨터(옵테론, 애슬론64, 인텔펜티엄 64버전)에서 동작 하는것을 의미한다.(단, 아이태니엄 버전은 IA64라고 표기함)

- x86_64

x86은 위에 언급한 CPU를 모두 포함

amd64 라고도 불리는 32 ~ 64비트 체계의 CPU 프로세서

x86의 64bit Server 혹은 PC에서 동작하는 것을 의미

x86-64는 x86 명령어 집합 아키텍처의 64비트 모임이다. x86-64 명령어 집합은 에뮬레이션 없이 인텔의 x86를 지원하며 AMD64로 이름 붙인 AMD에 의해 고안되었다. 이 아키텍처는 인텔 64라는 이름으로 인텔에 의해 복제되기도 했다. 이로써 x86-64 또는 x64의 이름을 일상적으로 사용하기에 이르렀다.

32비트 인텔 계열의 칩셋을 말합니다. 여기엔 x86, i386, i686 등이 속해 있습니다. 사실상 모두 같은 아키텍처를 가리키는 단어입니다만, 명목상 x86(인텔 8086 시리즈) 아키텍처 안에 i386(인텔 80386 호환)이나 i686(인텔 펜티엄 2 이상 호환)이 속한 것으로 볼수 있습니다. 즉 호환 CPU에 따라 숫자가 달라지는 것으로 볼수 있습니다(x86의 범위는 넓습니다. 심지어 이후 설명할 AMD64 역시 x86에 속합니다) 이는 IA-32(인텔 아키텍처-32)나 x32(단순히 32비트니까 32를 붙임)라고 부르기도 합니다.

64비트쪽으로 넘어오면 복잡해집니다. x86_64는 amd64 라고도 불림. CPU 명령어셋으로는 EM64T, IA-32e, Intel64 라고도 불림. 인텔이 64비트 관련 뻘짓을 한 영향이 큽니다.
