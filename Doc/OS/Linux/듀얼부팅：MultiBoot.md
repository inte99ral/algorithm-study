...
- 그래픽 드라이버
  ...
  #엔비디아 그래픽카드 드라이버 확인.
  #드라이버 없으면 No such file or directory
  $ cat /proc/driver/nvidia/version
  ```

  - $ sudo ubuntu-drivers devices `#설치가능한 NVIDIA 그래픽 카드 드라이버 목록`
  - $ sudo ubuntu-drivers autoinstall `#권장 드라이버 자동설치 or`
  - $ sudo apt install nvidia-driver-525 `#or 드라이버 목록 중에서 nvidia-driver-525 수동설치`
  - $ sudo reboot `#재부팅`

- 키보드 & 언어팩 설정

  - 우분투 상단바 우측을 클릭하면 톱니바퀴 모양의 설정 버튼이 있습니다.
  - 설정 `-⇀` 시스템 `-⇀` 지역 및 언어(Region & Language) `-⇀` 설치된 언어 관리(Manage Installed languages)
...
- 배드섹터 검사
  ...
  - OS가 설치된 디스크는 USB 부팅을 통해서 접근해야 합니다.
  - $ sudo fdisk -l `# 디스크 목록 확인`
  - $

- snap 및 dev/loop 제거
  - [참고링크](https://dragontory.tistory.com/445)
  - $ sudo apt autoremove --purge snapd
